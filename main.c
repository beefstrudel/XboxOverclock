...
// Define the absolute file path for settings file on the D:/ drive
#define SETTINGS_FILE_PATH "D:\\settings.bin"

// Function to save settings to the D:/ drive
void saveSettings(Settings settings) {
    FILE *file = fopen(SETTINGS_FILE_PATH, "wb");
    if (file != NULL) {
        fwrite(&settings, sizeof(Settings), 1, file);
        fclose(file);
        debugPrint("Settings saved to file: FSB = %d, NVCLK = %d, Memory Divider = %d\n", settings.fsb, settings.nvclk, settings.memoryDivider);
    } else {
        debugPrint("Error: Unable to open file for writing.\n");
    }
}

// Function to load settings from the D:/ drive
Settings loadSettings() {
    Settings settings = {0}; // Default values
...
int main(void) {
    ...
    if (SDL_Init(SDL_INIT_GAMECONTROLLER) != 0) {
        debugPrint("Unable to initialize SDL!\n");
        Sleep(10000);
        return 1;
    }
    ...
    debugPrint("Use \"Y\" to save overclock settings.\n");
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_CONTROLLERBUTTONDOWN) {
                ...
                case SDL_CONTROLLER_BUTTON_Y:
                    ...
                    saveSettings((Settings){wanted_fsb, wanted_nvclk, wanted_mp});
                    break;
                default:
                    break;
                }
